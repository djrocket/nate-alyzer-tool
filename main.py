...
def fetch_transcript_simple(request):
    headers = {
    ...
    try:
        ...
        if blob.exists():
            ...
            return jsonify({"transcript": cached_transcript, "source": "cache"}), 200, headers
        else:
            return jsonify({"status": f"Cache MISS for video ID: {video_id}. Live fetch is disabled."}), 404, headers
